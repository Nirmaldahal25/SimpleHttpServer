...
#define XSTRING(_NAME) STRING(_NAME)
const std::string path = XSTRING(TEMPLATES_DIR);
...
std::tuple<bool, enum Directory::Type> Files::checkForFileorFolder(const std::string &relative_path)
{
    ...
    if (exists)
    {
        ...
        if (std::filesystem::is_directory(path))
            return std::make_tuple<bool, enum Directory::Type>(true, Directory::Type::Folder);
        if (std::filesystem::is_regular_file(path))
            ;
        return std::make_tuple<bool, enum Directory::Type>(true, Directory::Type::File);
    }
